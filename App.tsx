...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white pb-20">
      <div className="relative h-[500px] md:h-[750px] w-full overflow-hidden bg-slate-900">
        <div 
        ...
        <header className="absolute inset-0 flex flex-col justify-end px-6 pb-20 max-w-7xl mx-auto w-full">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-10">
            <div className="flex items-center gap-6 group">
              <div className="w-16 h-16 md:w-20 md:h-20 bg-indigo-600 rounded-3xl flex items-center justify-center shadow-[0_25px_60px_rgba(79,70,229,0.5)] group-hover:rotate-3 transition-transform">
                <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-white"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/></svg>
              </div>
              <div>
                <h1 className="text-4xl md:text-7xl font-black text-slate-900 tracking-tighter leading-tight drop-shadow-2xl">
                  딥던전 커닝페이퍼
                </h1>
                <div className="mt-3 h-2 w-48 bg-indigo-600 rounded-full shadow-lg"></div>
              </div>
            </div>

            <div className="flex items-center bg-white/95 backdrop-blur-3xl rounded-[2rem] px-8 py-5 border border-slate-200 focus-within:border-indigo-500 focus-within:ring-8 focus-within:ring-indigo-500/10 transition-all shadow-[0_35px_60px_-15px_rgba(0,0,0,0.2)] w-full md:w-[480px]">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="text-slate-400"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
              <input 
                type="text" 
                placeholder="어떤 몬스터가 궁금하신가요?" 
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="bg-transparent border-none focus:outline-none ml-4 text-xl w-full text-slate-900 placeholder:text-slate-400 font-bold"
              />
            </div>
          </div>
        </header>
      ...
      <main className="max-w-7xl mx-auto px-6 -mt-10 relative z-20">
        <div className="flex flex-col md:flex-row items-center justify-between mb-16 bg-white/80 backdrop-blur-md p-6 rounded-[2.5rem] border border-slate-100 shadow-xl gap-6">
          <div className="flex flex-wrap justify-center md:justify-start gap-4">
            {DUNGEONS.map(d => (
              <button
                ...
                className={`px-8 py-4 rounded-2xl text-lg font-black transition-all transform active:scale-95 ${
        ...
        <div className="flex flex-col lg:flex-row gap-16">
          <aside className="lg:w-60 shrink-0">
            <div className="grid grid-cols-2 lg:grid-cols-1 gap-4 sticky top-10">
              {availableRanges.map(range => {
                ...
                return (
                  <button
                    ...
                    className={`px-6 py-5 rounded-2xl text-left transition-all border-2 group relative overflow-hidden font-black text-lg ${
          ...
          <section className="flex-1">
            <div className="mb-12">
              ...
              {searchQuery && (
                <p className="text-slate-400 mt-2 font-bold">'{searchQuery}'에 대한 검색 결과입니다.</p>
              )}
            </div>
            ...
            {filteredMonsters.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-40 bg-slate-50 rounded-[4rem] border-2 border-dashed border-slate-200">
                <div className="w-24 h-24 bg-white rounded-3xl flex items-center justify-center mb-10 text-slate-200 shadow-xl">
                ...
                <p className="text-lg text-slate-400 mt-4 font-bold">
